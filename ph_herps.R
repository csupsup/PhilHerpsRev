...
Types <- c("Diversity & Distribution", "Species Description & Taxonomy", "Biology & Ecology", "Evolution & Phylogenetics", "Conservation & Management")
...
```{r, num vs types}
#assignlabels
herps.stype <- mutate(herps.stype, lab = if_else(Types %in% "Diversity & Distribution", "Diversity & Distribution", if_else(Types %in% "Species Description & Taxonomy", "Species Description & Taxonomy", if_else(Types %in% "Biology & Ecology", "Biology & Ecology", if_else(Types %in% "Evolution & Phylogenetics", "Evolution & Phylogenetics", if_else(Types %in% "Conservation & Management", "Conservation & Management", "Not data"))))))

herps.stype$lab <- factor(herps.stype$lab, levels = c("Diversity & Distribution", "Species Description & Taxonomy", "Biology & Ecology", "Evolution & Phylogenetics", "Conservation & Management"))

herps.stype$Types <- factor(herps.stype$Types, levels = unique(herps.stype$Types))

#create stacked barplot
p.stype <- ggplot(herps.stype, aes(x = factor(Taxa, level = c("Amphibians", "Lizards", "Snakes", "Turtles", "Crocodiles", "Multitaxa")), y = Sum, fill = lab, label = Sum)) + geom_col(position = "stack") + geom_text(size = 4,aes(label = scales::comma(Sum)), position = position_stack(vjust = 0.5)) + theme(panel.background = element_blank(), axis.line = element_line(colour = "black"), legend.position = "", text = element_text(size=20), plot.title = element_text(size = 25, face = "bold"), axis.title=element_text(size=16, face="bold")) + scale_fill_manual(values = c("#abdda4", "#e6f598", "#fee08b", "#fdae61", "#f46d43")) + labs(fill="", x="", y="Number of papers") + ggtitle("(a)")
...
p.pie <- ggplot(d.pie[1:5, ], aes("", Percentage, fill = Types)) +
    ...
    theme(axis.line = element_blank(),
          ...
          plot.title = element_text(hjust = 0.5, color = "black", size = 25, face ="bold"), legend.position = c(0.0,0.97), ,text = element_text(size=20)) + ggtitle("(b)")
...
#create stacked barplot
p.for.loc <- ggplot(for.loc, aes(x = factor(Nat, level = c("Foreign", "Local")), y = Sum, fill = lab, label = Sum)) + geom_col(position = "stack") + geom_text(size = 4,aes(label = scales::comma(Sum)), position = position_stack(vjust = 0.5)) + theme(panel.background = element_blank(), axis.line = element_line(colour = "black"), legend.position = "top", legend.direction = "horizontal", legend.key.size = unit(0.5, 'cm'), legend.title=element_blank(), text = element_text(size=20), plot.title = element_text(size = 20, face = "bold"), axis.title=element_text(size=16, face="bold")) + scale_fill_manual(values = c("#fee08b", "#3288bd")) + labs(fill="", x="", y="Number of papers") + ggtitle("(b)")
...
p.new <- ggplot(newsp2, aes(Year, Value)) + geom_bar(data = newsp, aes(x=Year, y=Number),stat="identity", fill="#fdae61",colour="white") + geom_line(aes(linetype=Taxa), size = 1) + geom_point(aes(shape = Taxa), size = 2) + ylab("Number of new species") + xlab("Year") + scale_x_continuous("Year", labels = as.character(newsp$Year), breaks = newsp$Year) + theme(axis.text.x = element_text(angle = 70, vjust = 0.5), plot.title = element_text(size = 20, face = "bold"), legend.position = c(0.20,0.8), panel.background = element_blank(), panel.border = element_rect(colour = "black", fill=NA, size=0.5),legend.key=element_blank(), axis.title=element_text(size=16, face="bold"), text = element_text(size = 15)) + ggtitle("(a)")
...
#create stacked barplot
p.d.types <- ggplot(d.all, aes(x = Year, y = Freq, fill = lab, label = Freq)) + geom_col(position = "stack") + theme(panel.background = element_blank(), axis.line = element_line(colour = "black"), legend.position = c(0.75, 0.9), legend.direction = "vertical", legend.key.size = unit(0.4, 'cm'), legend.title=element_blank(), text = element_text(size=15), plot.title = element_text(size = 20, face = "bold"), axis.title=element_text(size=16, face="bold"), axis.text.x = element_text(angle = 70, vjust = 0.5)) + scale_fill_manual(values = c("#abdda4", "#5e4fa2")) + labs(fill="", x="", y="Number of papers") + ggtitle("") + scale_x_continuous("Year", labels = as.character(d.all$Year), breaks = d.all$Year) + ggtitle("(b)")
...
m.newsp <- ggplot() +
  ...
  labs(title = "") + scale_shape_manual(values = c(22,23,21)) + scale_fill_manual(values = p.cols) + theme(legend.position = "top", legend.direction = "vertical", legend.key = element_rect(fill = "transparent"), axis.line = element_blank(), panel.background = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.text.x=element_blank(), axis.ticks.x=element_blank(),axis.text.y=element_blank(), axis.ticks.y=element_blank(), plot.title = element_text(size = 15, face = "bold"), legend.title = element_blank(), legend.key.size = unit(0, 'lines')) + guides(fill=guide_legend(keywidth=0.1, keyheight=0.1, default.unit="inch"), shape = guide_legend(override.aes = list(size = 2.5))) + ggtitle("(a)")
...
m.ng <- ggplot() +
  ...
  labs(title = "") + scale_shape_manual(values = c(22,23)) + scale_fill_manual(values = p.cols) + theme(legend.position = "top", legend.direction = "vertical", legend.key = element_rect(fill = "transparent"), axis.line = element_blank(), panel.background = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.text.x=element_blank(), axis.ticks.x=element_blank(),axis.text.y=element_blank(), axis.ticks.y=element_blank(), plot.title = element_text(size = 15, face = "bold"), legend.title = element_blank(), legend.key.size = unit(0, 'lines')) + guides(fill=guide_legend(keywidth=0.1, keyheight=0.1, default.unit="inch"), shape = guide_legend(override.aes = list(size = 2.5))) + ggtitle("(b)")
...
ph.herps.s <- mutate(ph.herps.s, Paper = if_else(Type == 2, "Inventory Sites (2002-2022)", if_else(Type == 5, "Inventory Sites (Pre-2002)", if_else(Type == 6, "Inventory Sites (Curated Database)", "No data"))))

ph.herps.s$Paper <- factor(ph.herps.s$Paper, levels = c("Inventory Sites (2002-2022)", "Inventory Sites (Pre-2002)", "Inventory Sites (Curated Database)"))

p.cols <- c("#d53e4f", "#115deb", "#ffff2a")

m.s <- ggplot() +
  geom_sf(data = ph.adm, fill = "#818181", color = NA) +
  geom_sf(data = ph.herps.s, aes(shape = Paper, fill = Paper), size = 1.5) + labs(title = "") + scale_shape_manual(values = c(21,22,24)) + scale_fill_manual(values=p.cols) + theme(legend.position = "top", legend.direction = "vertical",legend.key = element_rect(fill = "transparent"), axis.line = element_blank(), panel.background = element_blank(), panel.grid.major = element_blank(),panel.grid.minor = element_blank(), axis.text.x=element_blank(), axis.ticks.x=element_blank(),axis.text.y=element_blank(), axis.ticks.y=element_blank(), plot.title = element_text(size = 15, face = "bold"), legend.title = element_blank(), legend.key.size = unit(0, 'lines'))  + ggtitle("(c)") + guides(fill=guide_legend(keywidth=0.1, keyheight=0.1, default.unit="inch"), shape = guide_legend(override.aes = list(size = 2.5)))

ggarrange(m.newsp, m.ng, m.s, ncol=3)

#legend.key.size = unit(0, 'lines') to reduce space between vertical legend keys

tiff("fig_4.tif", res=300, width = 9, height = 5, unit="in")
ggarrange(m.newsp, m.ng, m.s, ncol=3)
dev.off()
```
